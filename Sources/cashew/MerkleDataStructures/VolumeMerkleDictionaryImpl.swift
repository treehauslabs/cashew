/// Default concrete implementation of ``VolumeMerkleDictionary``. Wires the
/// dictionary's ``ChildType`` to ``VolumeRadixHeaderImpl`` so every internal
/// trie edge crosses a Volume boundary. Mutation logic (`inserting`,
/// `deleting`, `mutating`) is inherited from ``MerkleDictionary``'s protocol
/// extensions.
public struct VolumeMerkleDictionaryImpl<Value>: VolumeMerkleDictionary
